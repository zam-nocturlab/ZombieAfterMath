...
function PANEL:Think()
	if self.loading then
		if self.target and self.treeID then
			self.tree = nut.dialogue.list[self.treeID].tree
...
function PANEL:startTopic(topicID)
	...
	if topic.statement != "" then
		self:dialogue(LocalPlayer():getChar():getName(), topic.statement, true)
	end

	self:dialogue(self:targetName(), topic.response, false, nut.config.get("chatListenColor"), topicID)
end
...
function PANEL:dialogue(name, message, instant, color, topicID)
	...
	self.message:SetFont("nutChatFontBold")
...
vgui.Register("nutDialogue", PANEL, "EditablePanel")

net.Receive("nutDialogue", function()
